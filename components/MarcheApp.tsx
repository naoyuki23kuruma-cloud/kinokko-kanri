...
// ─── マルシェ選択・作成モーダル ───────────────────────────
function MarcheSelectModal({ marches, onSelect, onCreate, onClose }: {
  marches: Marche[]
        ...
        <div className="p-4 space-y-3 max-h-80 overflow-y-auto">
          ...
          {marches.map((m) => (
            <button key={m.id} onClick={() => onSelect(m)}
              className="w-full text-left bg-gray-50 hover:bg-blue-50 border border-gray-200 rounded-xl px-4 py-3 transition-colors">
...
export default function MarcheApp() {
  ...
  const handleCreateMarche = async (name: string, date: string) => {
    const { data, error } = await supabase.from('marches').insert({ name, date: date || null, status: 'planning' }).select().single()
    if (error) { showToast('作成に失敗しました', 'error'); return }
    await fetchMarches()
    handleSelectMarche(data)
  }

  const handleCloseMarche = async () => {
    if (!currentMarche) return
    if (!confirm(`「${currentMarche.name}」を終了済みにしますか？`)) return
    await supabase.from('marches').update({ status: 'closed' }).eq('id', currentMarche.id)
    const updated = { ...currentMarche, status: 'closed' as const }
    setCurrentMarche(updated)
    setMarches((prev) => prev.map((m) => m.id === updated.id ? updated : m))
    showToast('終了済みに変更しました')
  }
      ...
      <header className="bg-blue-600 text-white sticky top-0 z-30 shadow-md">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex gap-2 shrink-0">
            {currentMarche && currentMarche.status !== 'closed' && (
              <button onClick={handleCloseMarche} className="bg-blue-500 border border-blue-400 text-white text-xs px-2 py-1.5 rounded-lg">終了する</button>
            )}
